...
const navigate = useNavigate();
  ...
  const guardar = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://servicios-holisticos-backend.onrender.com/api/disponibilidad/terapeutas/disponibilidad",
  ...
  return (
    <div className="p-4 pt-12 max-w-xl mx-auto">
      <h2 className="text-xl font-semibold text-center text-[#333] mb-4">
        Agregá tu disponibilidad para este servicio 🌸
      </h2>
        ...
        {semana.map((dia, index) => (
          <div key={index} className="border rounded-xl p-2">
            ...
            <button
              ...
              className="bg-pink-500 text-white px-3 py-1 rounded-full text-xs w-full"
            >
              + Agregar horario
            </button>
      ...
      <button
        onClick={guardar}
        disabled={guardando}
        className="bg-pink-500 text-white px-5 py-2 rounded-full w-full"
      >
