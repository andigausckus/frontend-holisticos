...
const AdminPagos = () => {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold text-center mb-6">🧾 Panel de pagos</h1>

      {loading ? (
        <p className="text-center">Cargando reservas...</p>
      ) : reservas.length === 0 ? (
        <p className="text-center text-gray-600">No hay pagos pendientes.</p>
      ) : (
        <div className="space-y-6">
          {reservas.map((reserva) => (
            <div key={reserva._id} className="p-4 bg-white rounded-xl shadow">
              ...
              <p>
                <strong>Terapeuta:</strong> {reserva.terapeuta?.nombre}
              </p>
              <p>
                <strong>Servicio:</strong> {reserva.servicio?.titulo}
              </p>
              <p>
                <strong>Fecha:</strong>{" "}
