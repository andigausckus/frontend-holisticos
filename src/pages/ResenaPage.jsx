...
export default function ResenaPage() {
  ...
  if (!reserva || !reserva.servicioId) {
    console.error("⚠️ La reserva no tiene servicioId:", reserva);
    setError("No se encontró el servicio asociado a la reserva.");
    return;
  }

  try {
    const res = await fetch(`${API_URL}/servicios/${reserva.servicioId}/resena`, {
      method: "POST",
    ...
    if (res.ok) {
      ...
      console.error("❌ Error respuesta del backend:", errData);
